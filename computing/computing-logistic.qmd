...
This primer leads you down the path of least resistance to fitting this model, plotting the best-fitting S-curve in the single-predictor case, and plotting the decision boundary in the bivariate case. 
...
```
After doing some itchy algebra that you don't necessarily have to concern yourself with, we plot a scatter plot of our two predictors, color the points according to the type of the email, and then add a straight line (`geom_abline`) to visualize the decision boundary when we threshold $\text{Prob}(y=1)$ at 1/2:
...
```
According to our model, the probability is pretty much 100%, which is above any reasonable threshold we might choose. Send it to the trash!
