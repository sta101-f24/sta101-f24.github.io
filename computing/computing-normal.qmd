...
What is the probability that a randomly selected 65-year-old woman has bones that are less dense than pine? If we add an `L` argument to `normTail`, we can visualize the answer:
...
In order to compute the probability (area) under the bell curve of being less than 600 $mg/cm^3$, we can use the `pnorm` function:

```{r}
#| label: pnorm
pnorm(pine, mean = bone_density_mean, sd = bone_density_sd)
```
Notice how this may come in handy when we have to compute $p$-values, for instance.
